run.taxon.tests <- function(mapfile, otufile, outputfile, keep.pathways.file='data/pathways.to.keep.txt', control.baseline=F)
{
	minOTUInSamples = .001
	...
	if(control.baseline==F){
		...
		# Test 1 - Between subject comparison (DELTA_Taxon, Group A vs. Group B, use TimePoint==2 for other vars)
		# WHILE CONTROLLING FOR taxa_time1
		participant_ids <- unique(map$PARTICIPANTID)
		ix_t1 <- which(map$PRE_POST=="1" & (map$PARTICIPANTID %in% participant_ids))
		ix_t2 <- which(map$PRE_POST=="2" & (map$PARTICIPANTID %in% participant_ids))
		otu_delta <- otu[ix_t2,] - otu[ix_t1,]	

		ret3 <- test.otu.features2(otu_delta, map[ix_t2,], treatment="GROUP", controls=c("SEX","RACE","BMI"), otu.baseline=otu[ix_t1,])
		...
		# Test 2 - Redo the above, but instead of delta_taxa, just look at time 2 taxa
		# Between subject comparison (taxa_time2, Group A vs. Group B, use TimePoint==2 for other vars)
		# WHILE CONTROLLING FOR taxa_time1
		ret4 <- test.otu.features2(otu[ix_t2,], map[ix_t2,], treatment="GROUP", controls=c("SEX","RACE","BMI"), otu.baseline=otu[ix_t1,])

		cat("Group A vs. Group B at TimePoint 2 (while controlling for TimePoint 1)\n", file=outputfile, append=T)
